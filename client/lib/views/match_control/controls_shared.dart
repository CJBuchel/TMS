...
DataCell styledCell(String text, AnimationController controller, List<GameMatch> loadedMatches, {bool? isTable, bool? goodSig}) {
  return DataCell(
    AnimatedBuilder(
...
DataRow2 _styledRow(OnTable table, String matchNumber, List<Team> teams, AnimationController controller, List<GameMatch> loadedMatches,
  ...
  return DataRow2(cells: [
    ...
    styledCell(table.table, controller, loadedMatches, isTable: true, goodSig: goodSig),
...
List<DataRow2> getRows(
    List<GameMatch> matches, List<GameMatch> loadedMatches, List<Team> teams, AnimationController controller, Map<String, String> tableMap) {
  List<DataRow2> rows = [];
  for (var match in matches) {
    rows.add(_styledRow(match.onTableFirst, match.matchNumber, teams, controller, loadedMatches, tableMap));
    rows.add(_styledRow(match.onTableSecond, match.matchNumber, teams, controller, loadedMatches, tableMap));
  }
  return rows;
}
