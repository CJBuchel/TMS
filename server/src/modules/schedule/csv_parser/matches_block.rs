use crate::{
  ...
  modules::schedule::csv_parser::block::{BLOCK_MATCHES, Block},
};

#[derive(Debug, Clone)]
pub struct ScheduledTableAssignmentT {
  pub table_name: String,
  pub team_number: String,
}

#[derive(Debug, Clone)]
pub struct ScheduledMatchT {
  pub match_number: String,
  pub start_time: TmsDateTime,
  pub end_time: TmsDateTime,
  pub table_assignments: Vec<ScheduledTableAssignmentT>,
}

#[derive(Debug, Clone)]
pub struct ScheduledMatches {
  pub table_names: Vec<String>,
  pub matches: Vec<ScheduledMatchT>,
}

impl Block for ScheduledMatches {
  type Output = ScheduledMatches;

  fn from_csv(csv: &str) -> Result<Self::Output, String> {
    let block_lines = Self::get_block_lines(csv, BLOCK_MATCHES)?;
    ...
    for line in block_lines {
      ...
      match fields[0] {
        ...
        "Number of Ranking Matches" | "Number of Teams per Table" | "Number of Simultaneous Tables" => {}
        "Table Names" => {
          // from 1 to n_tables
          for i in 1..=n_tables {
    ...
    Ok(ScheduledMatches { table_names, matches })
  }
}
