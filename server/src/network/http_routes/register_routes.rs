...
use tms_utils::{TmsRespond, TmsRouteResponse, TmsClients, TmsClient};
// use rocket::serde::json::{Json};
...
pub fn register_route(clients: &State<TmsClients>, s_public_key: &State<Vec<u8>>, ws_port: &State<u16>, register_request: Json<RegisterRequest>) -> TmsRouteResponse<RegisterResponse, ()> {
  ...
  if clients.read().unwrap().contains_key(&user_id) {
    ...
    TmsRespond!(Status::AlreadyReported, RegisterResponse {
  ...
  TmsRespond!(Status::Ok, RegisterResponse {
    key: String::from_utf8(s_public_key.to_vec()).unwrap(),
    url: format!("ws://{:?}:{}/ws/{}", server_ip, ws_port, user_id)
  });
}
