...
pub async fn teams_get_route(clients: &State<TmsClients>, db: &State<std::sync::Arc<TmsDB>>, uuid: String) -> TmsRouteResponse<()> {
  ...
  for team_raw in db.get_data().await.teams.iter() {
...
pub async fn team_get_route(security: &State<Security>, clients: &State<TmsClients>, db: &State<std::sync::Arc<TmsDB>>, uuid: String, message: String) -> TmsRouteResponse<()> {
  ...
  match db.get_data().await.teams.get(&team_request.team_number).unwrap() {
    Some(t) => {
...
pub async fn team_update_route(message: String) -> TmsRouteResponse<()> {
  ...
  if check_permissions(clients, uuid, message.auth_token, perms).await {
    match db.get_data().await.teams.get(message.team_number.clone()).unwrap() {
      Some(t) => {
        let origin_team_number = t.team_number.clone();
        // update db
        match db.get_data().await.teams.update(origin_team_number.as_bytes(), message.team_data.team_number.as_bytes(), message.team_data.clone()) {
          Ok(_) => {},
          Err(e) => {
            match e {
  ...
  if check_permissions(clients, uuid, message.auth_token, perms).await {
    match db.get_data().await.teams.get(message.team_number.clone()).unwrap() {
      ...
      None => {
        ...
        // add team to db
        match db.get_data().await.teams.insert(message.team_number.clone().as_bytes(), team) {
          Ok(_) => {
            // update rankings
            if !update_rankings(db, clients).await {
              TmsRespond!(Status::BadRequest, "Failed to update rankings".to_string());
    ...
    for old_team in teams.clone() {
      if old_team.team_number == team.team_number {
        if old_team.ranking != team.ranking {
          ...
          match db.get_data().await.teams.update(team.team_number.as_bytes(), team.team_number.as_bytes(), team.clone()) {
            Ok(_) => {
              trigger_update = true;
            },
            Err(_) => {
  ...
  match res.match_number {
    Some(m) => {
      match res.table {
        Some(tb) => {
          // update match
          match db.get_data().await.matches.get(m.clone()).unwrap() {
            Some(game_match) => {
              ...
              for on_table in &mut updated_match.match_tables {
                if on_table.table == tb {
                  ...
                  match db.get_data().await.matches.update(m.as_bytes(), m.as_bytes(), updated_match.clone()) {
                    Ok(_) => {
                      // send updates to clients
                      tms_clients_ws_send(SocketMessage {
                        from_id: None,
  ...
  if check_permissions(clients, uuid, message.auth_token.clone(), perms).await {
    ...
    match db.get_data().await.teams.get(message.team_number.clone()).unwrap() {
      Some(mut t) => {
        ...
        let result = tms_event_service.read().await.scoring.validate(answers).await;
        ...
        match result {
          Some(validation) => {
            if validation.errors.is_empty() {
              ...
              // update the team in the database
              let _ = db.get_data().await.teams.update(t.team_number.as_bytes(), t.team_number.as_bytes(), t.clone());
...
pub async fn team_delete_route(message: String, tms_event_service: &State<TmsEventServiceArc>) -> TmsRouteResponse<()> {
  ...
  if check_permissions(clients, uuid, message.auth_token, perms).await {
    ...
    let exists: bool = match db.get_data().await.teams.get(message.team_number.clone()).unwrap() {
      Some(_) => true,
      None => false,
    };
    ...
    // remove team from db (using event service)
    let teams_service = tms_event_service.read().await.teams.clone();
    let result = teams_service.remove_team(message.team_number.clone()).await;
    ...
    match result {
      ...
      Err(_) => {
        ...
        TmsRespond!(Status::BadRequest, "Failed to remove team".to_string());
      }
    }
  }
