use warp::Filter;
use crate::{database::SharedDatabase, network::{robot_game_matches_load_matches_handler, robot_game_matches_ready_matches_handler, robot_game_matches_unload_matches_handler, robot_game_matches_unready_matches_handler, ClientMap}, services::SharedServices};
...
pub fn robot_game_matches_filter(clients: ClientMap, db: SharedDatabase, services: SharedServices) -> impl warp::Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
  ...
  let ready_matches = robot_game_matches_path.and(warp::path("ready_matches"))
    .and(warp::post())
    .and(with_services(services.clone()))
    .and(check_auth_token_filter(clients.clone()))
    .and(role_permission_filter(clients.clone(), db.clone(), vec!["admin", "head_referee"]))
  ...
  let unready_matches = robot_game_matches_path.and(warp::path("unready_matches"))
    .and(warp::post())
    .and(with_services(services.clone()))
    .and(check_auth_token_filter(clients.clone()))
    .and(role_permission_filter(clients.clone(), db.clone(), vec!["admin", "head_referee"]))
