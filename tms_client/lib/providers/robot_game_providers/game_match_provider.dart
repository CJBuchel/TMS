...
class GameMatchProvider extends EchoTreeProvider<String, GameMatch> {
  ...
  GameMatchProvider()
      : super(
            ...
            fromJsonString: (json) => GameMatch.fromJsonString(json: json));
    ...
    return timeSortedMatches.where((match) {
      if (match.category.category == category) {
        if (subCategories.isEmpty) {
          ...
          return match.category.subCategories
              .any((subCategory) => subCategories.contains(subCategory));
        }
      }
  ...
  String? getIdFromMatchNumber(String matchNumber) {
    return this
        .items
        .keys
        .firstWhereOrNull((key) => this.items[key]?.matchNumber == matchNumber);
  }

  GameMatch? get nextMatch =>
      matchesByTime.firstWhereOrNull((match) => !match.completed);

  List<GameMatch> getMatchesByTeamNumber(String teamNumber) {
    return matchesByTime.where((match) {
      return match.gameMatchTables
          .any((table) => table.teamNumber == teamNumber);
    }).toList();
  }
  ...
  GameMatch? getMatchByMatchNumber(String matchNumber) {
    return this
        .items
        .values
        .firstWhereOrNull((match) => match.matchNumber == matchNumber);
  }
  ...
  Future<int> insertGameMatch(String? matchNumber, GameMatch match) async {
    ...
    String? matchId =
        matchNumber != null ? getIdFromMatchNumber(matchNumber) : null;
  ...
  Future<int> removeTableFromMatch(String table, String matchNumber) async {
    ...
    if (match != null && matchId != null) {
      match.gameMatchTables
          .removeWhere((gameMatchTable) => gameMatchTable.table == table);
      return _service.insertMatch(matchId, match);
    }
  ...
  Future<int> addTableToMatch(
      String table, String teamNumberOnTable, String matchNumber) async {
    GameMatch? match = this
        .items
        .values
        .firstWhereOrNull((match) => match.matchNumber == matchNumber);
  ...
  Future<int> updateTableOnMatch({
    ...
    if (updatedMatchNumber != null && updatedMatchNumber != originMatchNumber) {
      // handle table switch
      GameMatch? originMatch = this
          .items
          .values
          .firstWhereOrNull((match) => match.matchNumber == originMatchNumber);
      GameMatch? updatedMatch = this
          .items
          .values
          .firstWhereOrNull((match) => match.matchNumber == updatedMatchNumber);
      ...
      if (originMatch != null &&
        ...
        if (updatedMatch.gameMatchTables
            .any((t) => t.table == updatedTable.table)) {
          return HttpStatus.badRequest;
        } else {
        ...
        int originStatus =
            await _service.insertMatch(originMatchId, originMatch);
        int updatedStatus =
            await _service.insertMatch(updatedMatchId, updatedMatch);
        return originStatus == HttpStatus.ok && updatedStatus == HttpStatus.ok
            ? HttpStatus.ok
